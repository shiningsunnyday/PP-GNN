...
for dataset_name in datasets_name:
    ...
    for repeat in range(args.repeat_num):
        ...
        if args.pretrain_task:
            ...
            for i,data in enumerate(data_list):
                preselect_anchor(args, data, layer_num=args.layer_num, anchor_num=args.anchor_num, device='cpu')
                data = data.to(device)
                data_list[i] = data
        ...
        for epoch in range(args.epoch_num):
            ...
            for id, data in enumerate(data_list[:effective_len]):
                if args.permute:
                    preselect_anchor(args, data, layer_num=args.layer_num, anchor_num=args.anchor_num, device=device, dataset=dataset_name)
                if args.pretrain_task:
                    out = model(G, data)
                else:
                    out = model(data)
    ...
    os.makedirs(f'results/{args.dataset}/{args.task}',exist_ok=True)
    with open(f'results/{args.dataset}/{args.task}/{hash_val}.txt', 'w+') as f:            
